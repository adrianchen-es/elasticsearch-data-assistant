...
class EnhancedSearchService:
    ...
    async def _generate_related_queries(self,
        ...
        # Fire-and-forget AI call for related query suggestions (we don't rely on response format yet)
            await self.ai_service.generate_elasticsearch_query(prompt, {}, return_debug=False)
        except Exception as exc:
            logger.debug(f"Failed to generate related queries: {exc}")
            return []

            # Extraction/parsing not implemented yet
            return []
